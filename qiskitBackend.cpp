...
void QiskitBackend::addBinaryExpression(BinaryExpression &expr, VariableList &g_variables, GateList &g_gates)
{
    ...
    if (expr.gate == "H" || expr.gate == "X" || expr.gate == "Y" || expr.gate == "Z")
    {
        ...
        for (int i = g_variables.vars[expr.variable].position; i < g_variables.vars[expr.variable].position + g_variables.vars[expr.variable].width; i++)
        {
            pythonOutput += "qc." + c;
            pythonOutput += "(" + std::to_string(i) + ")\n";
        }
    }
    if (expr.gate == "CX")
    {
        pythonOutput += "qc.append(XGate().control(" + std::to_string(g_variables.vars[expr.variable].width - 1) + "),";
        pythonOutput += "[";

        for (int i = g_variables.vars[expr.variable].position; i < g_variables.vars[expr.variable].position + g_variables.vars[expr.variable].width - 1; i++)
        {
            pythonOutput += std::to_string(i);
            pythonOutput += ",";
        }

        pythonOutput += std::to_string(g_variables.vars[expr.variable].position + g_variables.vars[expr.variable].width - 1);
        pythonOutput += "])\n";
    }
}
